...
def count_matches(pred_texts, gt_texts, convert_to_zhcn=False):
    ...
    for pred_text, gt_text in zip(pred_texts, gt_texts):
        if convert_to_zhcn:
            gt_text = convert(gt_text, 'zh-cn')
            pred_text = convert(pred_text, 'zh-cn')
...
def eval_ocr_metric(pred_texts, gt_texts, convert_to_zhcn=False):
    ...
    match_res = count_matches(pred_texts, gt_texts, convert_to_zhcn=convert_to_zhcn)
    eps = 1e-8
